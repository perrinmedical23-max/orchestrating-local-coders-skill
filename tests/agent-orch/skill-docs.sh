...
assert_contains "${SKILL_MD}" "examples/opencode/.agent-orch/providers.json"
assert_contains "${SKILL_MD}" "examples/opencode/.agent-orch/opencode-run.sh"
...
assert_contains "${SKILL_MD}" "Auth is manual and fail-fast"
assert_contains "${SKILL_MD}" "agent-orch\` must not start auth flows"
assert_contains "${SKILL_MD}" "Claude Opus 4.6 (Thinking)"
assert_contains "${SKILL_MD}" "Codex planning helper"
assert_contains "${SKILL_MD}" "not an \`agent-orch\` loop role"
assert_contains "${SKILL_MD}" "not a default automatic route"
assert_not_contains "${SKILL_MD}" "--role plan"
...
assert_contains "${ROUTING_GUIDELINES}" "Antigravity is an explicit-config provider template"
assert_contains "${ROUTING_GUIDELINES}" "not a default automatic route"
assert_contains "${ROUTING_GUIDELINES}" "Claude Code remains follow-up only"
assert_contains "${ROUTING_GUIDELINES}" "examples/opencode/.agent-orch/providers.json"
...
assert_contains "${ROUTING_GUIDELINES}" "examples/antigravity/.agent-orch/providers.json"
assert_contains "${ROUTING_GUIDELINES}" "examples/antigravity/.agent-orch/agy-run.sh"
assert_contains "${ROUTING_GUIDELINES}" "git -C <repo> add .agent-orch/providers.json .agent-orch/agy-run.sh"
assert_contains "${ROUTING_GUIDELINES}" "git -C <repo> commit -m"
assert_contains "${ROUTING_GUIDELINES}" "agent-orch provider check --provider antigravity --repo <repo>"
assert_contains "${ROUTING_GUIDELINES}" "Auth is manual and fail-fast"
assert_contains "${ROUTING_GUIDELINES}" "authenticate \`agy\` manually"
assert_contains "${ROUTING_GUIDELINES}" "must not start auth"
assert_contains "${ROUTING_GUIDELINES}" "Gemini 3.5 Flash (High)"
assert_contains "${ROUTING_GUIDELINES}" "AGENT_ORCH_ANTIGRAVITY_MODEL"
assert_contains "${ROUTING_GUIDELINES}" "Claude Opus 4.6 (Thinking)"
assert_contains "${ROUTING_GUIDELINES}" "Codex planning helper"
assert_contains "${ROUTING_GUIDELINES}" "not an \`agent-orch\` loop role"
assert_not_contains "${ROUTING_GUIDELINES}" "--role plan"
...
assert_contains "${README}" "fake OpenCode and fake Antigravity/agy fixtures"
assert_contains "${README}" "agent-orch provider check"
assert_contains "${README}" "examples/opencode/.agent-orch/providers.json"
assert_contains "${README}" "examples/opencode/.agent-orch/opencode-run.sh"
assert_contains "${README}" "examples/antigravity/.agent-orch/providers.json"
assert_contains "${README}" "examples/antigravity/.agent-orch/agy-run.sh"
assert_contains "${README}" "git -C <repo> add .agent-orch/providers.json .agent-orch/agy-run.sh"
assert_contains "${README}" "git -C <repo> commit -m"
...
assert_contains "${README}" "Antigravity is available only through the explicit per-repo provider template"
assert_contains "${README}" "not a default automatic route"
assert_contains "${README}" "Claude Code remains follow-up only"
...
assert_contains "${README}" "Gemini 3.5 Flash (High)"
assert_contains "${README}" "AGENT_ORCH_ANTIGRAVITY_MODEL"
assert_contains "${README}" "Auth is manual and fail-fast"
assert_contains "${README}" "agent-orch\` must not start auth flows"
assert_contains "${README}" "target repo \`HEAD\` before worktree execution"
assert_not_contains "${README}" "--role plan"

assert_contains "${RUN_ALL}" "skill-docs.sh"
assert_contains "${RUN_ALL}" "opencode-template.sh"
